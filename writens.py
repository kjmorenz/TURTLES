...
import calcnextphoton2 as simpcalc
import time as rt
import math
import numpy
...
def write(filepath, filedir, fullfilename, antibunch, diffuse, pulsed, numlines, maxlines, endtime,
    ...
    if ac > -1:
        if pulsed == 1:#untested
            ...
            AvgEmEvs = ac/((10**9)*sensitivity)#per ns
            k_excitation = AvgEms/AvgEmEvs - k_emission
            if k_excitation < 1:
                k_excitation = 1
                print("error: impossibly high count rate")
            phpers = 10**9/(k_excitation*absXsec)
    ...
    print(timestep)
    if pulsed == 1:
        k_excitation = 10**3/(phperpulse*absXsec*reprate)#*AvgEms) #ns/excitation
        experemperpulse = absXsec*phperpulse
        if experemperpulse > 1:
            experemperpulse = 1
        exsperpulse = absXsec*phperpulse*AvgEms
        AvgExEvs = exsperpulse*reprate*10**3
        experns = experemperpulse*taurep*AvgEms # comment *avgems out to make per em
        timestep = timestep/experns #ns per round
        if timestep < taurep:
            timestep = taurep
    ...
    #CW version:
        ...
        k_excitation = 10**9/(phpers*absXsec)#*AvgEms) #ns/excitation
        AvgEmEvs = AvgEms/(k_emission + k_excitation) # average sample emissions per second (in pHz)
        timestep = timestep/AvgEmEvs # in ns
        probex = 1
        exsperpulse = -1
        AvgExEvs = 10**9/k_excitation
    ...
    endround = timestep
    print(str(timestep) + " ns per round")
    ...
    diffouttime = (3*((beamwaist)**2)*(math.pi**2)*eta*r/(8*K_B*temp))*10**9
    ...
    avtime = (1/darkcounts)*(10**9) #average time between darkcount photons in ns given darkcounts in s^-1
    ...
    while (line < numlines or lastwritten < endtime) and line < maxlines:
        ...
        if compare == 0: #dftime == float("inf") and crashtime == float("inf") and compare == 0:
            dataphotons, lastphoton, numEms, nextIn, nextOut, diffsIn, diffsOut, ndiffsOut, countbright = simpcalc.nextphoton(lastphoton, sensitivity,
                                        ...
                                        diffsOut, ndiffsOut, countbright, probex)
    ...
    ##            print("wrote a line")
                line = line + 1
                if line%numlines==0:
                    print("time = " + str(lastwritten/10**9) + " s")
    ...
    print("Last photon time = " + str(int(max(lastphoton))/(10**9)) + " s")
    ...
    for i in lvars:
        pfile.write(i + " : " + str(lvars[i]) + "\n")
    pfile.write("\n")
    pfile.write("Last photon time = " + str(int(max(lastphoton))/(10**9)) + " s"+" \n")
    ...
    pfile.write("Time per round: " + str(timestep) + " ns  \n")
