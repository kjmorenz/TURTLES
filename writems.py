...
def write(filepath, filedir, fullfilename, antibunch, diffuse, pulsed, numlines, maxlines, endtime,
    ...
    phpers = laserpwr/(energyperphoton*1000) #factor of 1000 because laserpwr is in mW
    #make rounds average timestep emissions per round:
    print(timestep)
    if pulsed == 1:
        k_excitation = 1/(phperpulse*absXsec*(10**3)*reprate)#*AvgEms) #ms/excitation
        experemperpulse = absXsec*phperpulse
        if experemperpulse > 1:
            experemperpulse = 1
        exsperpulse = absXsec*phperpulse*AvgEms
        AvgExEvs = exsperpulse*reprate*10**6 #per s
        experms = experemperpulse*taurep*AvgEms # comment *avgems out to make per em
        timestep = timestep/experms #ms per round
        if timestep < taurep:
            timestep = taurep
        probex = 1- (1-absXsec)**phperpulse #1-probability of not being excited each pulse
    #CW version:
    else:
        k_excitation = 10**3/(phpers*absXsec)#*AvgEms) #ms/excitation
        AvgEmEvs = AvgEms/(k_emission + k_excitation) # average sample emissions per second (in mHz)
        timestep = timestep/AvgEmEvs # in ms
        probex = 1
        exsperpulse = -1
        AvgExEvs = 10**3/k_excitation #per second

    print("kex = " + str(k_excitation))
    print(timestep)
    if timestep < k_excitation + k_emission:
        timestep = 5*(k_excitation + k_emission)
    endround = timestep
    #timestep = timestep * 100
    print(str(timestep) + " ms per round")
    ...
    avtime = (1/darkcounts)*(10**3) #average time between darkcount photons in ms given darkcounts in s^-1
    darks = calc.darkcounts(avtime, dclen, 0, deadtime) #float array
    ...
    for i in lvars:
        pfile.write(i + " : " + str(lvars[i]) + "\n")
    pfile.write("\n")
    pfile.write("Last photon time = " + str(int(max(lastphoton))/(10**3)) + " s"+" \n")
    pfile.write("Last photon in file = " + str(photons[-1]) + " ms \n")
    ...
    pfile.write("deadtime = " + str(deadtime) + " ms  \n")
    ...
    pfile.write("Time per round: " + str(timestep) + " ms  \n")
##    pfile.write("Time limits for photon_gn = " + str(2**gnpwr) + "ms  \n")
